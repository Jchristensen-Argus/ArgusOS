...
Purpose:
    ...
    factory/packages/006_KNOWLEDGE_SERVICE.md, and
...
Startup Sequence:
    ...
    9. Register the seven core services (Configuration, Logger, Event
       Bus, Service Registry, Lifecycle Manager, Knowledge Service,
       Memory Service) in the Service Registry (identity/descriptive
       data only) and in the Lifecycle Manager, where each enters
       LifecycleState.REGISTERED. None of them are initialized or
       started by this package.
    10. Construct and start the Application.

Scope:
    This module implements only application startup infrastructure.
    No engines (Atlas, Cortex, Hermes, Navigator, Sentinel) are
    initialized here. Packages 003-007 register the Event Bus, Service
    Registry, Lifecycle Manager, Knowledge Service, and Memory Service
...
# The ArgusOS release this package targets, per the Package 007 work
# order header ("ArgusOS Version Target: v0.0.7"). Used as the version
# recorded on every core ServiceDescriptor registered during bootstrap.
CORE_SERVICES_VERSION = "0.0.7"
...
def _register_core_services(
    *,
    ...
    Package 007's equivalent requirement for the Memory Service.

    Each of Configuration, the Logger, the Event Bus, the Service
    Registry, the Lifecycle Manager, the Knowledge Service, and the
    Memory Service is recorded as a ServiceDescriptor (identity and
    ...
    which is the sole owner of runtime lifecycle state. Neither
    initialize() nor start() is called on the Lifecycle Manager for
    any of them: none of these seven classes implements IService yet
    (see factory/packages/007_MEMORY_SERVICE.md's Out of Scope section
    for why adopting IService remains a deliberately deferred, separate
    decision), and no package to date requires (or permits)
    automatically initializing or starting core services during
    bootstrap.
