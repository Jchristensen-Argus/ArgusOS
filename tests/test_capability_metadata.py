"""Unit tests for argus.capability.metadata.CapabilityMetadata."""

import dataclasses
import unittest
from datetime import datetime

from argus.capability import CAPABILITY_METADATA_VERSION, CapabilityMetadata
...
class DefaultsTests(unittest.TestCase):
    def test_defaults(self):
        ...
        self.assertEqual(metadata.version, CAPABILITY_METADATA_VERSION)
    ...
    def test_default_correlation_id_is_unique_per_instance(self):
        a = CapabilityMetadata()
        b = CapabilityMetadata()
        self.assertNotEqual(a.correlation_id, b.correlation_id)


class StyleMirrorTests(unittest.TestCase):
    def test_field_set_mirrors_context_planning_trace_task_relationship_and_execution_metadata(
        self,
    ):
        ...
        # This module mirrors those six's shape and declared order
        # instead - see metadata.py's own module docstring.
        field_names = {f.name for f in dataclasses.fields(CapabilityMetadata)}
...
class ExtraTests(unittest.TestCase):
    def test_extra_is_wrapped_in_mappingproxytype(self):
        metadata = CapabilityMetadata(extra={"a": 1})
    ...
    def test_extra_defensive_copy_not_shared_with_caller(self):
        original = {"a": 1}
        metadata = CapabilityMetadata(extra=original)
        original["a"] = 999
        self.assertEqual(metadata.extra["a"], 1)
...
class EqualityTests(unittest.TestCase):
    def test_equality_when_all_fields_match(self):
        created_at = datetime(2026, 1, 1)
        a = CapabilityMetadata(created_at=created_at, correlation_id="c1")
        b = CapabilityMetadata(created_at=created_at, correlation_id="c1")
        self.assertEqual(a, b)

    def test_not_equal_when_correlation_id_differs(self):
        created_at = datetime(2026, 1, 1)
        a = CapabilityMetadata(created_at=created_at, correlation_id="c1")
        b = CapabilityMetadata(created_at=created_at, correlation_id="c2")
