"""Unit tests for argus.task.metadata.TaskMetadata."""

import dataclasses
import unittest
from datetime import datetime

from argus.task import TASK_METADATA_VERSION, TaskMetadata
...
class DefaultsTests(unittest.TestCase):
    def test_defaults(self):
        ...
        self.assertEqual(metadata.version, TASK_METADATA_VERSION)
...
class StyleMirrorTests(unittest.TestCase):
    def test_field_set_mirrors_context_planning_and_trace_metadata(self):
        ...
        # all use. This module mirrors those three's shape and
        # declared order instead - see metadata.py's own module
        # docstring.
        field_names = {f.name for f in dataclasses.fields(TaskMetadata)}
...
class ExtraTests(unittest.TestCase):
    def test_extra_is_wrapped_in_mappingproxytype(self):
        metadata = TaskMetadata(extra={"a": 1})
    ...
    def test_extra_defensive_copy_not_shared_with_caller(self):
        original = {"a": 1}
        metadata = TaskMetadata(extra=original)
        original["a"] = 999
        self.assertEqual(metadata.extra["a"], 1)
...
class EqualityTests(unittest.TestCase):
    def test_equality_when_all_fields_match(self):
        created_at = datetime(2026, 1, 1)
        a = TaskMetadata(created_at=created_at, correlation_id="c1")
        b = TaskMetadata(created_at=created_at, correlation_id="c1")
        self.assertEqual(a, b)

    def test_not_equal_when_correlation_id_differs(self):
        created_at = datetime(2026, 1, 1)
        a = TaskMetadata(created_at=created_at, correlation_id="c1")
        b = TaskMetadata(created_at=created_at, correlation_id="c2")
