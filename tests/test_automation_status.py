"""Unit tests for argus.automation.status.AutomationStatus."""

import unittest

from argus.automation import AutomationStatus


class MembersTests(unittest.TestCase):
    def test_has_exactly_four_members(self):
        self.assertEqual(
            {member.name for member in AutomationStatus},
            {"ACTIVE", "PAUSED", "DISABLED", "ARCHIVED"},
        )

    def test_values_are_lowercase_strings_matching_member_names(self):
        for member in AutomationStatus:
...
class NoTransitionLogicTests(unittest.TestCase):
    def test_is_a_plain_enum_not_a_str_subclass(self):
        self.assertFalse(issubclass(AutomationStatus, str))

    def test_defines_no_public_methods_beyond_enum_machinery(self):
        public_methods = [
            name
            for name in vars(AutomationStatus)
            if not name.startswith("_")
            and callable(getattr(AutomationStatus, name))
            and name not in AutomationStatus.__members__
        ]
...
class RoundTripTests(unittest.TestCase):
    def test_every_member_round_trips_through_its_own_value(self):
        for member in AutomationStatus:
            self.assertIs(AutomationStatus(member.value), member)

    def test_members_are_distinct(self):
        values = [member.value for member in AutomationStatus]
        self.assertEqual(len(values), len(set(values)))


class EqualityAndIdentityTests(unittest.TestCase):
    def test_same_member_is_identical_across_references(self):
        self.assertIs(AutomationStatus.ACTIVE, AutomationStatus.ACTIVE)

    def test_different_members_are_not_equal(self):
        self.assertNotEqual(AutomationStatus.ACTIVE, AutomationStatus.ARCHIVED)
