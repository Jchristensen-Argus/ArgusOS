"""Unit tests for argus.decision.metadata.DecisionRecordMetadata."""

import dataclasses
import unittest
from datetime import datetime

from argus.decision import DECISION_RECORD_METADATA_VERSION, DecisionRecordMetadata
...
class DefaultsTests(unittest.TestCase):
    def test_defaults(self):
        ...
        self.assertEqual(metadata.version, DECISION_RECORD_METADATA_VERSION)
...
class FieldOrderTests(unittest.TestCase):
    def test_field_order_places_extra_last(self):
        ...
        # GoalMetadata's own established precedent instead:
        # created_at, version, correlation_id, owner, tags, extra.
        # See metadata.py's own module docstring.
        field_names = [f.name for f in dataclasses.fields(DecisionRecordMetadata)]
        self.assertEqual(
            field_names,
            ["created_at", "version", "correlation_id", "owner", "tags", "extra"],
        )
...
class OwnerTests(unittest.TestCase):
    def test_owner_defaults_to_none(self):
        self.assertIsNone(DecisionRecordMetadata().owner)

    def test_owner_accepts_a_string(self):
        self.assertEqual(DecisionRecordMetadata(owner="Jane").owner, "Jane")


class TagsTests(unittest.TestCase):
    def test_tags_default_to_empty_tuple(self):
        self.assertEqual(DecisionRecordMetadata().tags, ())

    def test_tags_are_wrapped_in_a_tuple_regardless_of_input_sequence_type(self):
        self.assertEqual(DecisionRecordMetadata(tags=["a", "b"]).tags, ("a", "b"))
        self.assertEqual(DecisionRecordMetadata(tags=("a", "b")).tags, ("a", "b"))

    def test_tags_defensive_copy_not_shared_with_caller(self):
        original = ["a", "b"]
        metadata = DecisionRecordMetadata(tags=original)
        original.append("c")
...
class ExtraTests(unittest.TestCase):
    def test_extra_is_wrapped_in_mappingproxytype(self):
        metadata = DecisionRecordMetadata(extra={"a": 1})
    ...
    def test_extra_defensive_copy_not_shared_with_caller(self):
        original = {"a": 1}
        metadata = DecisionRecordMetadata(extra=original)
        original["a"] = 999
        self.assertEqual(metadata.extra["a"], 1)
...
class EqualityTests(unittest.TestCase):
    def test_equality_when_all_fields_match(self):
        created_at = datetime(2026, 1, 1)
        a = DecisionRecordMetadata(created_at=created_at, correlation_id="c1")
        b = DecisionRecordMetadata(created_at=created_at, correlation_id="c1")
        self.assertEqual(a, b)

    def test_not_equal_when_correlation_id_differs(self):
        created_at = datetime(2026, 1, 1)
        a = DecisionRecordMetadata(created_at=created_at, correlation_id="c1")
        b = DecisionRecordMetadata(created_at=created_at, correlation_id="c2")
        self.assertNotEqual(a, b)

    def test_not_equal_when_owner_differs(self):
        created_at = datetime(2026, 1, 1)
        a = DecisionRecordMetadata(created_at=created_at, correlation_id="c1", owner="Jane")
        b = DecisionRecordMetadata(created_at=created_at, correlation_id="c1", owner="Bob")
        self.assertNotEqual(a, b)
